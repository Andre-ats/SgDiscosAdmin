...
export function AtualizarProduto() {
    ...
    const [embalagem, setEmbalagem] = useState<EnumEmbalagemProduto>();
    const [condicao, setCondicao] = useState<EnumCondicao>();
    const [status, setStatus] = useState<EnumStatusProduto>();
    const [quantidadeProduto, setQuantidadeProduto] = useState<number>();
    const [precoProduto, setPrecoProduto] = useState("");
    const [formatoProduto, setFormatoProduto] = useState<EnumFormatoProduto>();
    ...
    async function getProdutosByIdHandler() {
        try {
            ...
            const produtoEncontrado = await getProdutoById(id as string);

            setProduto(produtoEncontrado);

            setNomeProduto(produtoEncontrado.nomeProduto);
            setNomeArtistaBanda(produtoEncontrado.nomeArtistaBandaProduto);
            setDescricao(produtoEncontrado.descricaoProduto);
            setEmpresaGravadora(produtoEncontrado.empresaProduto);
            setOrigem(produtoEncontrado.origemProduto);
            setAnoLancamento(produtoEncontrado.anoLancamentoProduto);
            setCodigoDeBarra(produtoEncontrado.codigoBarra);

            setEmbalagem(produtoEncontrado.embalagemProduto);
            setCondicao(produtoEncontrado.condicao);
            setStatus(produtoEncontrado.statusProduto);
            setFormatoProduto(produtoEncontrado.formatoProduto);
            setTipoDeAlbum(produtoEncontrado.tipoDeAlbum);

            setQuantidadeProduto(produtoEncontrado.quantidadeProduto);
            setQuantidadeDeCancoes(
                produtoEncontrado.quantidadeDeCancoesProduto
            );
            setQuantidadeDeDiscos(produtoEncontrado.quantidadeDiscos);

            setPrecoProduto(
                produtoEncontrado.precoProduto.toFixed(2).replace(".", ",")
            );

            setGenerosMusicaisProduto(
                produtoEncontrado.generosMusicaisProduto
            );
        } catch (error) {
            console.error(error);
        } finally {
